...
[![Vibecode Hackathon](https://img.shields.io/badge/Vibecode_Hackathon-2025-7928CA?style=for-the-badge&logo=codeigniter&logoColor=white)](https://github.com/)
...
[![Supabase](https://img.shields.io/badge/Supabase_Realtime-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white)](https://supabase.com/)
[![Google Gemini 1.5](https://img.shields.io/badge/Gemini_1.5_Flash-4285F4?style=for-the-badge&logo=google&logoColor=white)](https://ai.google.dev/)
[![Cohere AI](https://img.shields.io/badge/Cohere_NLP-39594C?style=for-the-badge&logo=cohere&logoColor=white)](https://cohere.com/)
...
```mermaid
flowchart TB
    subgraph Clients[" Client Layer (Zero-Hardware Web Clients) "]
        Visitor["📱 Visitor Virtual Ticket<br/>/ticket/[id] (Mobile Web)"]
        Display["📺 Public TV Display<br/>/display (Signage Board + TTS)"]
        Staff["⚡ Staff Operator Cockpit<br/>/counter (Multi-Counter Console)"]
        Admin["📊 Executive Dashboard<br/>/admin (AI Reports & Analytics)"]
    end

    subgraph EdgeLayer[" Next.js 15 App Router & API Gateway "]
        Routes["RESTful Route Handlers<br/>/api/tokens • /api/queues • /api/counters"]
        Security["Security Barrier<br/>Turnstile CAPTCHA • DOMPurify • Zod Validation"]
    end

    subgraph RealTimePipeline[" Real-Time Synchronization Engine "]
        SupabaseRT["Supabase Realtime Channel<br/>PostgreSQL CDC + WebSocket Broadcasts"]
        SSEBroadcaster["In-Memory State & Event Broadcaster"]
    end

    subgraph CoreEngine[" Core Domain Logic & Queue Engine "]
        QueueStateMachine["Atomic Token State Machine<br/>lib/queue/engine.ts"]
        ETACalculator["Weighted Moving Average ETA Engine<br/>lib/eta/calculator.ts"]
        AuditLedger["Immutable Audit Trail<br/>TokenEvent Log"]
    end

    subgraph AIEngine[" AI Intelligence Hub "]
        Gemini["Google Gemini 1.5 Flash<br/>Grounded Operational Analyst"]
        Cohere["Cohere NLP Chat API<br/>Visitor Intent & Department Triage"]
    end

    subgraph Persistence[" Persistent Database Layer "]
        Postgres[(PostgreSQL via Supabase Pooler)]
        PrismaORM["Prisma ORM 6.3 Engine"]
    end

    %% Flow Connections
    Visitor -->|Join Queue / Emergency Request| Security
    Staff -->|Call / Serve / Transfer / Pause| Security
    Admin -->|Queue Config / Trigger AI Report| Security
    
    Security --> Routes
    Routes --> QueueStateMachine
    QueueStateMachine --> PrismaORM
    PrismaORM --> Postgres
    
    QueueStateMachine --> ETACalculator
    QueueStateMachine --> AuditLedger
    
    Postgres -.->|CDC Trigger| SupabaseRT
    QueueStateMachine -.->|Direct Event Broadcast| SupabaseRT
    
    SupabaseRT ==>|Sub-second WebSocket Sync| Visitor
    SupabaseRT ==>|Sub-second WebSocket Sync| Display
    SupabaseRT ==>|Sub-second WebSocket Sync| Staff
    SupabaseRT ==>|Sub-second WebSocket Sync| Admin
    
    Admin -->|Request Operational Report| Gemini
    Visitor -->|Natural Language Inquiry| Cohere
    Gemini -.->|Grounded JSON Report| Admin
    Cohere -.->|Department Recommendations| Visitor
```
...
```mermaid
stateDiagram-v2
    [*] --> CREATED: Visitor Joins via QR / NFC
    CREATED --> WAITING: Token Generated (Sequence # Assigned)
    
    state WAITING {
        [*] --> StandardPriority
        StandardPriority --> EmergencyPending: Request Triage Escalation
        EmergencyPending --> PriorityElevated: Staff Approves
        EmergencyPending --> StandardPriority: Staff Rejects
    }
    
    WAITING --> CALLED: Operator Clicks "Call Next" (Audio Chime + TTS)
    CALLED --> SERVING: Visitor Arrives at Counter
    CALLED --> NO_SHOW: Grace Period Expires (Timeout)
    CALLED --> RECALLED: Operator Re-Announces Token
    
    SERVING --> COMPLETED: Service Finalized
    SERVING --> TRANSFERRED: Re-routed to another Department Queue
    
    WAITING --> CANCELLED: Visitor Leaves Virtual Queue
    TRANSFERRED --> WAITING: Re-indexed with Priority Preservation
...
erDiagram
    ...
    QUEUE ||--o{ COUNTER : allocates
    QUEUE ||--o{ TOKEN_EVENT : emits
    TOKEN ||--o| EMERGENCY_REQUEST : escalates
    TOKEN ||--o{ TOKEN_EVENT : triggers
    COUNTER ||--o{ TOKEN : serves

    QUEUE {
        string id PK
        string name
        string code UK "e.g. A, B, C"
        string department
        string status "ACTIVE | PAUSED | CLOSED"
        int estimatedServiceTime "mins"
        datetime createdAt
    }

    COUNTER {
        int number PK "Counter 1..6"
        string name
        string queueId FK
        string status "AVAILABLE | BUSY | PAUSED | CLOSED"
        string operatorName
    ...
    TOKEN {
        string id PK
        string displayNumber "e.g. A-101"
        int sequenceNumber
        string queueId FK
        string visitorSessionId
        string visitorName
        string purpose
        string status "WAITING | CALLED | SERVING | COMPLETED | CANCELLED | NO_SHOW"
        int position
        string priority "STANDARD | EMERGENCY"
        string counterId FK
        datetime createdAt
        datetime calledAt
    ...
    TOKEN_EVENT {
        ...
        string tokenId FK
        string queueId FK
        string eventType "TOKEN_CREATED | TOKEN_CALLED | EMERGENCY_PROMOTED..."
        string actor
        json metadata
        datetime createdAt
    }
